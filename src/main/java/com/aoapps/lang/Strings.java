...
public final class Strings {
    ...
    if (days == 0) {
        ...
        if (minutes == 0) {
          if (seconds == 0) {
            if (time == 0) {
              ...
              sb.append(time).append(time == 1 ? " millisecond" : " milliseconds");
            }
          } else {
      ...
      if (hours == 0) {
        if (minutes == 0) {
          ...
          sb.append(days).append(days == 1 ? " day and " : " days and ").append(minutes).append(minutes == 1 ? " minute" : " minutes");
        }
      } else {
        if (minutes == 0) {
          sb.append(days).append(days == 1 ? " day and " : " days and ").append(hours).append(hours == 1 ? " hour" : " hours");
        } else {
          sb.append(days).append(days == 1 ? " day, " : " days, ").append(hours).append(hours == 1 ? " hour and " : " hours and ").append(minutes).append(minutes == 1 ? " minute" : " minutes");
        }
      }
    }
    return sb.toString();
  ...
  public static String trim(String value) {
    ...
    while ((st < len) && isWhitespace(cp = value.codePointBefore(len))) {
      ...
      if (len < st) {
        // Just in case strangely overlapping invalid code points
        len = st;
      }
    }
    assert st  >= 0;
